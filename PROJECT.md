...
- 10.06.2026: API-/Config-Versionierung im Transcript-/Config-Contract entfernt (kein `contractVersion`, keine `version`-Pflicht in `azure.json`).
- 17.06.2026: Azure-Konfiguration vereinfacht: Speech-Key liegt direkt in `config/azure.json` (`speechKey`) statt über Env-Var.
...
- 23.06.2026: Neues Azure-Konfigurationsformular umgesetzt: initial sichtbar bei fehlender `config/azure.json`, zusätzlich dauerhaft über den Einstellungsbereich aufrufbar.
...
- 06.06.2026, 07:xx UTC: T-103 umgesetzt und auf Review gesetzt: Config-Contract + Beispielkonfigurationen eingeführt (`src/shared/config-contract.ts`, `config/azure.example.json`, `config/user-settings.example.json`), lokale Runtime-Config in `.gitignore` ergänzt und Fallback-Regeln in README dokumentiert.
